...
const paginatedMessages = await messembed.listMessages({
  chatId: '...',
  after: new Date('...'), // Отправленные после. Необязательный параметр
  before: new Date('...'), // Отправленные до. Необязательный параметр
  offset: 0, // Необязательный параметр
  limit: 100, // Необязательный параметр
});
```
...
Этот метод удаляет все сообщения в указанном чате для текущего пользователя, но сам чат остается. Сообщения не удаляются для собеседника.
